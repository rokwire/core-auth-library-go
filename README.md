...
### `coreservice`
The `coreservice` package provides the `CoreService` type which contains the configurations and helper functions to utilize certain functions implemented by the ROKWIRE Core Building Block. Examples of these functions include getting the IDs of accounts deleted within a set amount of time ago, and registering permissions with the Core Building Block.
